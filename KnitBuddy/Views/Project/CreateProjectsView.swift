	...
	var body: some View {
		VStack {
			...
			.overlay(alignment: .trailing) {
				Text("\(name.count)/18")
					.foregroundStyle(name.count > 18 ? .red : .black)
					.padding(.trailing, 10)
			}
			.padding(.bottom, 20)
			.onChange(of: name) {
				if name.count > 22 {
					name = String(name.prefix(22))
				}
			}
			VStack {
				Text("Choose a Color")
			...
			Button {
				...
				.foregroundStyle(name.isEmpty || name.count > 18 ? .lightBlack.opacity(0.3) : colorFromRawValue(selectedColor))
			}
			.disabled(name.isEmpty || name.count > 18)
		}
		.padding()
